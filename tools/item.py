...
class SingleItem(Exportable):
    ...
    def get_color(self):
        val = struct.unpack("<H", self._data[4:6])[0]
        colors = []
        for i in range(4):
            colors.append((val >> (12 - i * 4)) & 0xF)
        return colors
...
class Item(Exportable):
    ...
    def set_id(self, new_id):
        ...
        if self.items:
            ...
            self.items = [SingleItem(struct.pack("<H", new_id) + "\x00" * 5 + "\x0c")]
            self.count = 1
            self.stack = True
